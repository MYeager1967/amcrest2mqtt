...
# Read env variables
amcrest_host = "192.168.30.50"  # os.getenv("AMCREST_HOST")
amcrest_port = 80  # int(os.getenv("AMCREST_PORT") or 80)
amcrest_username = "admin"  # os.getenv("AMCREST_USERNAME") or "admin"
amcrest_password = "89Bmw325i"  # os.getenv("AMCREST_PASSWORD")

storage_poll_interval = int(os.getenv("STORAGE_POLL_INTERVAL") or 0)
device_name = os.getenv("DEVICE_NAME")

mqtt_host = "192.168.10.30"  # os.getenv("MQTT_HOST") or "localhost"
mqtt_qos = int(os.getenv("MQTT_QOS") or 0)
mqtt_port = int(os.getenv("MQTT_PORT") or 1883)
mqtt_username = "homeassistant"  # os.getenv("MQTT_USERNAME")
mqtt_password = "A-10Warthog"  # os.getenv("MQTT_PASSWORD")  # can be None
mqtt_tls_enabled = False  # os.getenv("MQTT_TLS_ENABLED") == "true"
...
topics = {
    ...
    "ivs": f"amcrest2mqtt/{serial_number}/ivs/",
...
async def main():
    try:
        async for code, payload in camera.async_event_actions("All"):
            ...
            elif (is_ad110 and code == "AlarmLocal") or (
                code == "VideoMotion" and not is_ad110
            ):
                log("AlarmLocal")
                motion_payload = "on" if payload["action"] == "Start" else "off"
